...
export class Camera extends BaseCamera {
     ...
     */
    // czh:编辑器里也调用这个绘制rt，Camera里面调用这个函数的函数没有触发
    ...
    */
    ...
    destroy(destroyChild: boolean = true): void {
        // czh ：Fixed an issue where the ide crashed due to an error 
        // when switching between scenes when the camera was not turned on, 
        // hdr was checked on the component or post-processing was used
        //this._needInternalRenderTexture() && (!this._internalRenderTexture?._inPool) && this._internalRenderTexture ? RenderTexture.recoverToPool(this._internalRenderTexture) : null;
